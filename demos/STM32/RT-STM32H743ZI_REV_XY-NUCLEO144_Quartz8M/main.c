...
static const ADCConversionGroup adcgrpcfg = {
  ...
  .cfgr2 = 0U,
  .ccr = 0U,
  .pcsel = ADC_SELMASK_IN4 | ADC_SELMASK_IN7,
  .ltr1 = 0U,
  .htr1 = 0U,
  .ltr2 = 0U,
  .htr2 = 0U,
  ...
  .sqr = {
    ADC_SQR1_SQ1_N(ADC_CHANNEL_IN4) | ADC_SQR1_SQ2_N(ADC_CHANNEL_IN7),
    0U,
    0U,
    0U
  }
...
static void mux_set(uint8_t sel) {
  palWriteLine(MUX_S0_LINE, (sel & 0x01U) != 0U);
  palWriteLine(MUX_S1_LINE, (sel & 0x02U) != 0U);
  palWriteLine(MUX_S2_LINE, (sel & 0x04U) != 0U);
}

static void adc_read_pair(adcsample_t *dest) {
  adcsample_t dummy[2];
...
int main(void) {
  ...
  while (true) {
    for (uint8_t ch = 0; ch < 8U; ch++) {
      ...
      adc_read_pair(samples);
      values[ch][0] = samples[0];
      values[ch][1] = samples[1];
    }

    drv_display_clear();
    drv_display_draw_text(0, 0, "MUX PC4/PA7");
    for (uint8_t i = 0; i < 8U; i++) {
      snprintf(line, sizeof(line), "C%u:%5u %5u",
               (unsigned)i, (unsigned)values[i][0], (unsigned)values[i][1]);
